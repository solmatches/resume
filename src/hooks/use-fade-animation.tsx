...
export function useFadeAnimation({ target, options = {} }: UseIntersectionProps): void {
  const intersection = useCallback(
    async (entries: IntersectionObserverEntry[], self: IntersectionObserver) => {
  ...
  }, [target, options.root, options.threshold, options.rootMargin, intersection]);
}
